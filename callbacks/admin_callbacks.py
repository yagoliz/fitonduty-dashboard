...
def update_selected_view_info(group_id, participant_id, show_all, date_range):
    ...
    if date_range:
        if date_range["mode"] == "single":
            ...
            if single_date:
                date_str = datetime.strptime(single_date, '%Y-%m-%d').strftime('%B %d, %Y') if isinstance(single_date, str) else single_date.strftime('%B %d, %Y')
            ...
            if date_range["mode"] == "7days":
                ...
                if start_date and end_date:
                    start_str = datetime.strptime(start_date, '%Y-%m-%d').strftime('%b %d, %Y') if isinstance(start_date, str) else start_date.strftime('%b %d, %Y')
                    end_str = datetime.strptime(end_date, '%Y-%m-%d').strftime('%b %d, %Y') if isinstance(end_date, str) else end_date.strftime('%b %d, %Y')
...
def update_data_visualizations(group_id, show_all, participant_id, date_range):
    ...
    if mode == "single":
        ...
        if mode == "7days":
            start_date = (datetime.strptime(end_date, '%Y-%m-%d').date() - timedelta(days=6)).isoformat() if isinstance(end_date, str) else (end_date - timedelta(days=6)).isoformat()
        elif mode == "30days":
            start_date = (datetime.strptime(end_date, '%Y-%m-%d').date() - timedelta(days=29)).isoformat() if isinstance(end_date, str) else (end_date - timedelta(days=29)).isoformat()
        elif mode == "all":
            # Use a year ago as default for "all time"
            start_date = (datetime.strptime(end_date, '%Y-%m-%d').date() - timedelta(days=365)).isoformat() if isinstance(end_date, str) else (end_date - timedelta(days=365)).isoformat()
        else:  # For custom range
            start_date = date_range.get("start_date")
            if not start_date:
                start_date = (datetime.strptime(end_date, '%Y-%m-%d').date() - timedelta(days=30)).isoformat() if isinstance(end_date, str) else (end_date - timedelta(days=30)).isoformat()
    ...
    if participant_id:
        ...
        return create_participant_detail_data(participant_id, start_date, end_date, mode)
...
def create_participant_detail_data(participant_id, start_date, end_date, mode):
    ...
    try:
        ...
        if df_history.empty:
            return html.Div("No data available for the selected participant and date range")
